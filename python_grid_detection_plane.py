...
def process_images_in_folder(json_path, folder_path, output_folder=None):
    ...
    for filename in tqdm(image_files, desc="Processing images"):
        ...
        try:
            ...
            # Optionally, store the error message in the JSON data
            images_json_data[base_name] = {'error': str(e)}
            continue
...
def detect_grid(input_img):
    ...
    try:
        ...
        # 1. 이미지 로드 및 전처리
        # -----------------------------------
        # input_img_array = 'image_data.mat'
        original_img, gray_img, blurred_img, binary_img = util_plane.load_and_preprocess_image(input_img)

        # -----------------------------------
        # 2. initial grid points detection
        # -----------------------------------
        horizontal_mask, vertical_mask, centroids = util_plane.extract_joints(binary_img)

        # -----------------------------------
        # 3. 실린더 영역 찾기(blob detection => contour => rect)
        # -----------------------------------
        # max_contour, mask_contour = util_plane.detect_largest_blob(original_img, binary_img, clipLimit=4.5)
        max_contour, mask_contour = util_plane.get_convex_hull(original_img, expansion_pixels=5, visualize=False)

        # -----------------------------------
        # 4. 실린더 영역 내 grid points 필터링 및 중심점(center) 선정
        # -----------------------------------
        final_img, cylinder_centroids, center_point, radius = util_plane.find_cylinder_centroids_and_center(
        ...
        # -----------------------------------
        mask_roi_h, mask_roi_v, circle_radius = util_plane.mask_roi_around_center(
            horizontal_mask, vertical_mask, mask_contour, original_img
        )

        # -----------------------------------
        # 6. grid points 보정 및 json 생성(center: (,), points: [좌표 인덱스 'id': (x,y), 실질 좌표값 'x': 'y': ])
        # -----------------------------------
        col_img, result_json, rows_updated, cols_updated = util_plane.color_and_expand_lines(
            mask_roi_h, mask_roi_v, circle_radius, max_contour, mask_contour, original_img, cylinder_centroids
        )
...
if __name__ == "__main__":
    ...
    folder_path = r"data\input\laser_plane"
    # folder_path = r'data\input\test_plane'
    output_folder = r"C:\Users\parkchaeho\MATLAB Drive\matlab_0120_pch\delete\data\output\test_plane\plane_0206"
    result_json = process_images_in_folder(json_path, folder_path, output_folder)
